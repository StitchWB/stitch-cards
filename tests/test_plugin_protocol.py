...
Spawns ``python -m stitch_cards`` (no host dependency) and walks the
JSON-RPC 2.0 line protocol: init → _migrate_db → ping → generate_cards →
...
MODULE = "stitch_cards"
PLUGIN_ID = "stitch-cards"
TEST_COMMAND = "generate_cards"
TEST_PARAMS = {"bin": "411111"}
...
def test_lifecycle_init_migrate_ping_command_shutdown() -> None:
    ...
    readonly command with realistic params, and graceful shutdown."""
